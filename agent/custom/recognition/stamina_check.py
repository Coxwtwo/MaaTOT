...
@AgentServer.custom_recognition("StaminaCheck")
class StaminaCheck(CustomRecognition):
    """
    StaminaCheck 自定义识别类
    ...
    def analyze(
        ...
        logger.info("StaminaCheck: 正在检查当前体力数值...")
        ...
        try:
            ...
            if not res or not res.all_results:
                logger.warning(f"StaminaCheck: 在区域 {roi_list} 内未识别到任何文本")
                return None
            
            # 3. 结果处理：将所有识别到的文本块拼接（应对不同字号导致的文本切分问题，如 "10" 和 "/160"）
            full_text = "".join([item.text for item in res.all_results])
            logger.info(f"StaminaCheck: OCR 识别结果为 '{full_text}'")
            
            # 4. 数值解析：使用正则表达式提取第一个数字序列
            match = re.search(r"(\d+)", full_text)
            if not match:
                logger.warning(f"StaminaCheck: 无法从文本 '{full_text}' 中解析出有效的体力数字")
                return None
            ...
            if current_stamina < threshold:
                logger.info(f"StaminaCheck: 体力不足 ({current_stamina} < {threshold}) -> 触发补给流程")
                # 返回一个有效的虚拟坐标盒，告知 Pipeline 该节点已“匹配成功”，从而触发后续的 next 链路
                return CustomRecognition.AnalyzeResult(box=[0, 0, 1, 1], detail={"stamina": current_stamina})
                
            # 如果体力充足，则判定为“匹配失败” (No Hit)
            # 这将促使 Pipeline 尝试 next 列表中的低优先级项（如 Click_取消 逻辑）
            logger.info(f"StaminaCheck: 体力充足 ({current_stamina} >= {threshold})，跳过能量饮料补给动作")
            return None
            
        except Exception as e:
            logger.exception(f"StaminaCheck: 运行过程中发生未知异常: {e}")
            return None
