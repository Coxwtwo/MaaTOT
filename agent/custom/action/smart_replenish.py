...
class SmartReplenish(CustomAction):
    ...
    def run(
        ...
        res_ap = context.run_recognition_direct(
            JRecognitionType.OCR, JOCR(roi=ACTIONPOINT_VALUE_ROI), img
        )
        ...
        for cfg in DRINK_CONFIGS:
            ...
            match_res = context.run_recognition_direct(
                ...
                JTemplateMatch(
                    template=cfg["template"], threshold=0.7, roi=DRINK_SEARCH_ROI
                ),
                img,
            )
            ...
            abs_x, abs_y, abs_w, abs_h = (
                (box.x, box.y, box.w, box.h) if hasattr(box, "x") else box
            )
            ...
            res_exp = context.run_recognition_direct(
                JRecognitionType.OCR, JOCR(roi=exp_roi), img
            )
            exp_weight = 9999
            ...
            res_count = context.run_recognition_direct(
                JRecognitionType.OCR, JOCR(roi=count_roi), img
            )

            inventory = 1  # 默认有库存
            if res_count and res_count.all_results:
                m = re.search(
                    r"(\d+)", "".join([it.text for it in res_count.all_results])
                )
                if m:
        ...
        available = []
        if s120:
            available.append(("120", s120))
        if s60:
            available.append(("60", s60))
        if s30:
            available.append(("30", s30))

        if available:
            # 按权重排序
            available.sort(key=lambda x: x[1]["exp"])

            # 如果存在权重相同的饮料，优先选择体力值更大的
            if len(available) >= 2 and available[0][1]["exp"] == available[1][1]["exp"]:
                # 获取所有权重相同的饮料
                same_weight = [
                    x for x in available if x[1]["exp"] == available[0][1]["exp"]
                ]
                # 体力值优先级：120 > 60 > 30
                priority = {"120": 3, "60": 2, "30": 1}
                target = max(same_weight, key=lambda x: priority[x[0]])[1]
            else:
        ...
        if target:
            logger.info(
                f"SmartReplenish: 选定 {target['name']} (保质期权重: {target['exp']})"
            )
            ...
            context.override_pipeline({target["node"]: {"roi": dynamic_roi}})
