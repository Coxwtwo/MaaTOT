...
> ⚠️ **分发节点 next 顺序**：`[JumpBack]Click_对话框标识_你我之间` 和 `[JumpBack]Click_跳过每日签到弹窗_你我之间` 必须排在首页识别 `Flag_你我之间首页_X` **之前**。否则点击返回后画面尚未稳定时，后续动作可能被突然弹出的对话框/每日签到弹窗卡住。分发节点和首页 Flag 的 next 中都按此顺序排列。
...
flowchart LR
    ...
    n6 -------->|next| n13["返回主界面"]
    n7 -------->|next| n13
    n11 -->|next| n16["Click_开始处理外勤委托<br>(日常委托)"]
    n12 -->|next| n30["Click_开始处理外勤委托_庭审委托<br>(庭审委托)"]

    n16 -->|next| n8["Flag_体力不足弹窗<br>(在interface中设定是否开启)"]
    n8 -->|next| n14["Click_取消喝饮料"]
    n14 ---->|next| n13["返回主界面"]

    n16 -->|next| n9["Flag_战力不足<br>(日常委托 → 任务终止)"]
    n9 ----->|next| n13["返回主界面"]
    n16 -->|next| n17["Flag_疲劳值上限弹窗"]
    n16 -->|next| n18["Click_居中开始辩论<br>(utils 通用辩论链)"]
    n16 -.->|JumpBack| z["Flag_体力补充弹窗"]

    n30 -->|next| n8b["Flag_体力不足弹窗"]
    n8b -->|next| n14
    n30 -->|next| n31["Flag_战力不足_庭审委托<br>(ColorMatch 红字 → 继续辩论)"]
    n30 -->|next| n32["Click_居中开始辩论_庭审委托"]
    n30 -->|next| n17
    n30 -.->|JumpBack| z

    n31 -->|next| n32
    n32 -->|next| n33["Flag_自动出卡已开启_庭审委托"]
    n32 -.->|JumpBack| n23b["Click_开启自动出卡"]
    n33 -->|next| n34["Flag_暂时休庭_庭审委托<br>(体力不足 → 休庭)"]
    n33 -->|next| n25b["Click_点击继续"]
    n33 -->|next| n26b["Click_结束"]
    n33 -.->|JumpBack| n27b["Flag_自动出卡中"]
    n34 -->|next| n26b
...
由于 `Click_日常委托` 和 `Click_庭审委托` 使用 `[JumpBack]` 前缀标记，所以遇到战力不足或者体力不足时，虽然游戏回到了主界面但任务逻辑会回到 `Click_进入外勤区域` 节点再次进行识别。因此我们在 `Click_进入外勤区域` 的 next 中添加了 `Flag_主界面任务` 作为任务出口（注意它必须排在 next 列表**首位**）。

**日常委托**使用通用辩论链，后续节点位于 `utils.json`，辩论成功则停留在外勤界面，辩论失败则回到主界面。

**庭审委托**走独立的 `Click_开始处理外勤委托_庭审委托` 辩论链：识别到战力不足（ColorMatch 红色字体）时**不终止任务**，而是输出橙色提示「战力不足，庭审委托可能进行多次辩论」后继续进入辩论。因此庭审委托可通过任务选项 **「自动补足体力数值」** 设置体力补充阈值——体力补充阈值越高，可重复辩论的次数越多。
...
未名市周任务负责自动完成每周随机任务。周任务上限为 4 个，每次随机分配。
...
`Flag_进行中` 是**核心分发节点**，`next` 列表包含 多个 `[JumpBack]` 子任务识别条目，按从上到下优先级匹配：
...
2. **地图导航层** (`Swipe_任务名_1`)：用**地点定位图**做 recognition 确认已到达正确区域，同时执行第一步滑动
3. **滑动步骤层** (`Swipe_任务名_2+`)：用**移动按钮**做 recognition 检查画面无中断，然后执行滑动。`next` 列表统一携带三条中断处理 + 下一步滑动 + 跳回任务列表

由于未名市周任务在追踪任务后，其他任务的触发点也不会消失，**移动途中可能碰到其他任务的触发点**，为了减少开发调试工作，我们按以下逻辑处理周任务

**周任务执行时，不区分当前在执行哪一个子任务**

我们把执行完所有移动(Swipe_X_N)视为完成一次子任务。而移动途中的**对话框**、**奖励弹窗**、**辩论弹窗**等都视为中断移动的干扰项。

不管 **任务A 移动途中**触发的是 **任务A** 或是 **任务B** 的弹窗，都会处理中断后继续移动，直到完成所有移动，然后检查任务列表。如果 **任务A 没有正常完成**，那么任务列表中会**再次识别到** 任务A ，**再次执行** 任务A 
...
每个滑动步骤节点(Swipe_任务名_N)拥有统一的 next 模板：
...
滑动步骤1 用地点定位图检查确认在中央商业街初始位置，滑动步骤2 用移动按钮检查（确认无打断）后滑动，滑动后触发辩论或奖励。
...
flowchart LR
    ...
    n3 -->|next| n_done
    subgraph 中断处理
        Ghost:::hidden ~~~|JumpBack| d1["Click_对话框标识_未名市"]
        Ghost ~~~|JumpBack| d2["Click_关闭奖励弹窗_未名市"]
        Ghost ~~~|JumpBack| d3["Click_前往辩论_未名市"]
    end
```
...
**对话选项**：需要在中央商业街滑动 5 步，最后一步触发对话选项"在老奶奶左边/右边"。这是目前**唯一需要选择对话选项**的任务。图中省略了中断处理。
...
flowchart LR
    ...
    s5 -.->|JumpBack| n_option["Click_对话选项_歌舞未名_未名市<br>(OCR: 在老奶奶左边/右边)"]
```

#### 3. 维护中央广场2（长滑动链）

**多步滑动链**：需要连续 9 步滑动才能完成。早期版本曾使用 OCR 条件分支（按 0/3、1/3、2/3 进度文字走三次不同路径），现已简化为统一滑动路径。图中省略了中断处理。


```mermaid
flowchart LR
    n1["Flag_维护中央广场2<br>(OCR: 维护中央广场（二)"] -->|next| n2["Swipe_维护中央广场2_1<br>(定位图: 中央广场定位图<br>下)"]
    n1 -.->|JumpBack| n_exit["Flag_退出未名市任务"]
    n1 -.->|JumpBack| n_reset["Flag_重置位置_中央广场"]

    n2 -->|next| n3["Swipe_维护中央广场2_2<br>(移动按钮检查<br>右)"]
    n3 -->|next| n4["Swipe_维护中央广场2_3<br>(下)"]
    n4 -->|next| n5["Swipe_维护中央广场2_4<br>(右)"]
    n5 -->|next| n6["Swipe_维护中央广场2_5<br>(右)"]
    n6 -->|next| n7["Swipe_维护中央广场2_6<br>(上)"]
    n7 -->|next| n8["Swipe_维护中央广场2_7<br>(上)"]
    n8 -->|next| n9["Swipe_维护中央广场2_8<br>(下)"]
    n9 -->|next| n10["Swipe_维护中央广场2_9<br>(左)"]
    n10 -->|next| n_done["Click_未名市任务<br>(回任务列表)"]

```

#### 4. 绶带鸟的礼物（多子任务递进）

**多子任务递进**：三个子任务分散在不同地点，完成一个后下一个出现在任务列表中。图中省略了中断处理。

```mermaid
flowchart LR
    n_scan["Flag_进行中<br>扫描任务列表"] -->|匹配| g3["Flag_绶带鸟的礼物1<br>(地点: 中央商业街)"]
    n_scan -->|匹配| g2["Flag_绶带鸟的礼物2<br>(地点: 中央商业街)"]
    n_scan -->|匹配| g1["Flag_绶带鸟的礼物3<br>(地点: 忒弥斯律所前台)"]

    g3 --> s3["Swipe_1_1~5<br>(5步 → Click_未名市任务)"]
    g2 --> s2["Swipe_2_1~2<br>(2步 → Click_未名市任务)"]
    g1 --> s1["Swipe_3_1~2<br>(2步 → Click_未名市任务)"]

    s3 --> back["回到 Flag_进行中<br>重新扫描任务列表"]
    s2 --> back
    s1 --> back
```
...
**组合识别**：同时满足 OCR 识别到"律所前台"文字 + TemplateMatch 匹配前台定位图，才确认在前台。防止在律所其他楼层误触发。图中省略了中断处理。
...
flowchart LR
    ...
    s2 -->|next| s2b["Click_暴力装修_前往二楼办公区<br>(OCR: 二楼办公区)"]
...
| `Click_对话选项_歌舞未名_未名市` | OCR (`在老奶奶左边`/`右边`) | 仅歌舞未名使用 |
